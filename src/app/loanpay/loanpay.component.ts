...
@Component({
  ...
  templateUrl: './loanpay.component.html',
  styleUrls: ['./loanpay.component.scss']
})
...
public async getAccount(): Promise<string> {
    ...
    this.cs.loan_get_count().then(game =>{
      ...
      game.forEach(item => {
        this.cs.get_loan_id(item).then(add =>{
          ...
          {
            this.Loan_get.push({"id":add,"lender_add":obj[1],"amt":obj[4]+" Ξ","settlement":obj[5],"next_settle_time":obj[6],"month":obj[7],"bal_loan":obj[8]+" Ξ","current_inst":obj[9]+" Ξ"})
          });
        })
      });
    })
 ...
 ngOnDestroy() {
  ...
  submit(){
    ...
    this.cs.pay_due(this.pay_due).then((res)=>{
    console.log(res);
